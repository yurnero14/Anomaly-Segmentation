...
class iouEval:
    ...
    def reset (self):
        ...
        #for each class
        self.tp = torch.zeros(classes).double() #true positive
        self.fp = torch.zeros(classes).double() #false positive
        self.fn = torch.zeros(classes).double() #false negative
    ...
    def addBatch(self, x, y):   #x=preds, y=targets
        ...
        if (x.size(1) == 1):
            x_onehot = torch.zeros(x.size(0), self.nClasses, x.size(2), x.size(3))  # create the tensor for 1-hot-encoding
            if x.is_cuda: # if x is on cuda
                x_onehot = x_onehot.cuda() # put 1-hot-encoding on cuda
            x_onehot.scatter_(1, x, 1).float() # scatter along dimansion 1 (of size self.nClasses), the (1d-)tensor x, putting in scattered position value 1
        else:
            x_onehot = x.float()
    ...
    def getIoU(self):
        num = self.tp
        den = self.tp + self.fp + self.fn + 1e-15 #1e-15 avoid overflow
        iou = num / den
